...
- Added a `/readyz` database readiness probe (branch `codex/db-readiness-check`): `store.Store` gained `Ping() error` (memory always healthy; gorm pings the underlying `*sql.DB`), `GET /readyz` returns 200 when reachable / 503 `数据库不可用` when not, and `/healthz` stays a static liveness probe. Tests cover ready (memory + gorm) and the 503 path via an embedded-interface failing stub; `internal/http` 77.0% -> 77.9%. This closes the production-readiness hardening for the persistence phase.
- Started the P0 real-ization phase with real JWT authentication (branch `codex/jwt-auth`): a new stdlib-only `internal/auth` (HS256, no new dependency) issues/verifies JWTs (sub=userID, iat, exp; 7-day TTL) with the secret from `PAWMIGO_JWT_SECRET` (dev default, must override in production). Login now returns a signed JWT; `middleware.Auth()` verifies the JWT and no longer accepts the static dev token. The dev login still works (the handler resolves the user id via the existing store seam and re-issues a JWT). `internal/auth` 79.6%, middleware 100%. Remaining for real login: WeChat code2session (separate task).
- Added a config file workflow (branch `codex/env-config-loader`): `backend/api/.env.example` template (active vars + planned WeChat/COS/Redis/map placeholders) plus a stdlib-only `.env` loader in `internal/config` that `cmd/server` runs at startup. Splits on the first `=` (DSN-safe), handles comments/`export`/quotes, treats a missing file as optional, never overrides real env vars or logs values. The real `backend/api/.env` is gitignored. The user fills `backend/api/.env` with real credentials (e.g. `WECHAT_APP_ID`/`WECHAT_APP_SECRET`) — these load automatically; WECHAT_* are not read by code until the code2session task. `internal/config` 87.2%.
- Implemented real WeChat code2session login (branch `codex/wechat-login`): new `internal/wechat` (stdlib client) + `store.EnsureUserByOpenID` (memory + gorm find-or-create by openid, in the shared contract suite). Login exchanges `{code}` -> openid -> user -> JWT when WeChat is configured, else keeps the dev fallback; `NewRouter` got variadic options for test injection. Tests stub the API (no network). Verified the user's real credentials out-of-band: jscode2session with a dummy code returned errcode 40029 ("invalid code"), proving the AppID/Secret are valid and the endpoint is reachable — full end-to-end needs a real `code` from the mini-program. Note: the user initially filled secrets into the tracked `.env.example`; these were moved to the gitignored `backend/api/.env` and the template restored before any commit, so no secret was ever committed/pushed.
- Implemented image upload via Tencent COS presigned credentials (branch `codex/cos-upload`): new `internal/cos` (stdlib-only COS presigned PUT signing) and a protected `POST /api/v1/upload/credential` returning `{uploadUrl, fileUrl, objectKey, expiresIn}` (503 when COS unconfigured), with `NewRouter` option `WithCOSSigner`. Offline tests cover signing/escaping/200/400/503/401. Validated signature correctness against the user's real COS bucket out-of-band via a throwaway smoke (presigned PUT 200, cleanup DELETE 204; the smoke file was removed before commit). The COS creds live only in the gitignored `backend/api/.env`. Frontend still needs wiring to call this endpoint and upload via the returned URL.
- Started A-4 (frontend↔backend e2e): wired the mini-program remote login to a real WeChat code (branch `codex/remote-wechat-login`). In remote mode `authService.login` now calls `Taro.login()` (named alias import, WeChat-safe) and posts the real `code` to `/auth/wechat-login`; mock mode and token storage/header untouched. Verified `pnpm typecheck`, `pnpm build:weapp` (compiled with no warnings — note: the Taro build only runs in the main environment; the codex sandbox panics in the Rust system-configuration crate, so build:weapp must be verified outside that sandbox), `pnpm test` (97/97).
- Continued A-4: wired real image upload into the post-create flow through the service seam. New `UploadService.uploadImage(tempFilePath)` on the contract/`AppServices`; mock echoes the local temp path (offline), remote runs the backend's two-step presigned direct upload (infer ext jpg/jpeg/png/webp → `POST /upload/credential {ext}` → read bytes via `getFileSystemManager().readFile` → raw `PUT` ArrayBuffer to `uploadUrl`, no Authorization header → return `fileUrl`; throws `图片上传失败` on non-2xx). Post-create page swapped the mock-image cycler for `chooseImage` → `uploadService.uploadImage` (9-cap kept). Taro fake gained `chooseImage`/`getFileSystemManager`; +2 remote upload tests. Scoped to post images only (pet avatar / report images NOT yet wired — note: a real presigned PUT can't use `wx.uploadFile` (multipart); it must raw-PUT bytes, as implemented). Verified `pnpm typecheck`, `pnpm build:weapp` (no warnings, main env), `pnpm test` (99/99).
- Closed out the A-4 image surface (PR #13): reused `uploadService` on the remaining pickers with no service-layer changes. Pet create + edit avatars now tap `chooseImage({count:1})` → upload → `setAvatarUrl`, display/submit `avatarUrl || DEFAULT_AVATARS[type]` (edit backfills the existing avatar). Report evidence got a 3-image picker with thumbnails (tap to remove) and submits real `images` instead of `[]`; reported-target preview unchanged. Report SCSS added layout-only grid/thumb rules (no WXSS color fallback needed). All images (post / pet avatar / report) now flow through the same seam. Verified `pnpm typecheck`, `pnpm build:weapp` (no warnings, main env), `pnpm test` (99/99). Remaining A-4: a full device/simulator e2e against a locally running gorm+MySQL backend (user-run; sandbox can't bind `:8080` or run the Taro build).
- Opened A-3 (admin web). User decisions (this session): stack = Vite + React + TS + antd v5 + `@ant-design/pro-components` + react-router-dom (NOT Ant Design Pro/umi); first slice = mock-first admin shell. Scaffolded `admin/pet-social-admin` (Manager ran the Vite create + `pnpm install` in the main env, since the codex sandbox blocks network installs; antd pinned to v5 for the pro-components peer). codex then authored the shell on top (PR #14): service seam (contracts + mock default `admin`/`admin123` + remote fetch client for `/api/v1/admin` with `{code,message,data}` envelope + Bearer `admin_token`, switched by `VITE_API_MODE`), token helpers, login page, public/protected route guards, `ProLayout` with 9-route menu + logout, dashboard with mock stats, and 8 `PageContainer+Empty` placeholder pages. `ConfigProvider` zhCN + green theme. Verified main-env `pnpm typecheck`/`lint`/`build` all green. Build/typecheck/lint run fine in the codex sandbox too (plain Vite/tsc, no Taro native panic) once deps are installed. Known gap: `@ant-design/icons` not installed (menu uses text badges). Next A-3 slices: install `@ant-design/icons` + build real ProTable CRUD pages (users/pets/posts/reports first), and add backend admin endpoints (admin auth + RBAC + resource APIs under `/api/v1/admin`) so remote mode works.
...
A-4 (frontend↔backend e2e) is in progress: remote login and post-image upload are now wired through the service seam. The next A-4 step is a full device/simulator end-to-end run against a locally running gorm+MySQL backend (the user must run the server + WeChat Devtools since the codex sandbox can't bind `:8080` or run the Taro/WeChat build). To run it: start `PAWMIGO_STORE=gorm go run ./cmd/server` (loads real WeChat/COS creds from the gitignored `backend/api/.env`), build the mini program in remote mode (`TARO_APP_API_MODE=remote TARO_APP_API_BASE_URL=http://localhost:8080 pnpm build:weapp`), open `dist` in WeChat Devtools, and exercise login → create pet → post with a real photo. Optionally extend the upload service to the pet-avatar and report-image pickers next.
